...
import pathlib
import pytest
import test_common.fs.temp as temp
...
TEST_CERTIFICATE_NAME = "Dummy certificate for automated testing (Don't use for sign anything important. Use for " \
                        "testing signing in automated workflows.) <dummy_test@gmail.com>"
TEST_CERTIFICATE_PASSWORD_PATHNAME = os.path.join(TEST_CERTIFICATE_FOLDER, "test_certificate_password.txt")
TEST_PUB_CERTIFICATE_PATHNAME = os.path.join(TEST_CERTIFICATE_FOLDER, "test_pub.txt")
TEST_CERTIFICATE_PASSWORD = pathlib.Path(TEST_CERTIFICATE_PASSWORD_PATHNAME).read_text()


def test_rpm_keys_loaded(fingerprint: str):
    """ Context manager to load test keys temporally in RPM database and remove then after tests. """
    myrpm.import_public_key(TEST_PUB_CERTIFICATE_PATHNAME)
    yield
...
def test_sign(temp_dir):
    ...
    with test_mygpg.test_keys_loaded() as _:
...
def test_is_valid_signature():
    ...
    with test_mygpg.test_keys_loaded() as fingerprint:
        with test_rpm_keys_loaded(fingerprint):
